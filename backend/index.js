...
import GoodModal from './models/GoodModal.js';
dotenv.config();
...
app.post("/api/goods/create", async (req, res) => {
	const createdGood = await GoodModal.create({
		vendoreсode: req.body.vendoreсode,
		brand: req.body.brand,
		name: req.body.name,
		weight: req.body.weight,
		height: req.body.height
	})
	res.send(createdGood)
})

app.get('/api/goods/all', async (req, res) => {
	try {
		const goods = await GoodModal.find();// Используем метод find() без аргументов, чтобы получить все записи
		res.send(goods);
	} catch(error){
